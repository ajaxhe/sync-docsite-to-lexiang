...
class LexiangConnector:
    ...
    def move_entry(self, entry_id, new_parent_id, after=None):
        """
        移动 entry 到新父节点（用于删除同步的「回收站」模式）。
        乐享 MCP 无删除工具，删除同步 = 移入回收站文件夹。
        """
        args = {"entry_id": entry_id, "parent_id": new_parent_id}
        if after:
            args["after"] = after
        data = self.call_tool("entry_move_entry", args)
        return data

    def create_page(self, space_id, name, parent_id=None):
        """创建空 page entry（返回 entry_id），正文由公共上传 CLI 覆盖。"""
        args = {"entry_type": "page", "name": name}
        if parent_id:
            args["parent_entry_id"] = parent_id
        data = self.call_tool("entry_create_entry", args)
        entry = data.get("data", {}).get("entry", {}) if isinstance(data, dict) else {}
        eid = entry.get("id")
        if not eid:
            raise LexiangError(f"创建页面失败: {json.dumps(data, ensure_ascii=False)[:200]}")
        return eid
